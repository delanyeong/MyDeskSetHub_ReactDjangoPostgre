...
function Dashboard() {
  ...
  async function deleteHandler () {
    ...
    try {
      const response = await axios.delete(`http://localhost:8000/api/posts/${params.uuid}/delete`)
      ...
      >
            ...
            <div className="w-0 flex-1">
              <dl>
                ...
                <h1 class="font-bold text-center">{posts.name}</h1>
                  ...
                  <div className="text-sm text-gray-900">{posts.brand}</div>
                  <div className="text-sm text-gray-900">Brand: {posts.description}</div>
        ...
        <div className="flex justify-between bg-gray-50 px-5 py-3">
          ...
          <div className="flex space-x-2">
            ...
            <div className="w-px h-6 bg-gray-400" >
                  {globalState.userId == state.listingInfo.seller ? (<button type="button">
                  <BookmarkIcon
                    className="h-6 w-6 text-gray-400"
                    aria-hidden="true"
